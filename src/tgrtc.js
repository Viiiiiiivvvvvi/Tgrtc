...
class TGRTC {
  ...
  constructor() {
    ...
    this.relayMode = new Map(); // userId -> boolean (whether using relay for this connection)
    this.connectionTimeout = 10000; // 10 seconds timeout for direct connection
    this.connectionTimers = new Map(); // userId -> timer
  }
  ...
  _connectSignalingServer(serverUrl = window.location.origin) {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Handle relay-related events
        this.socket.on('use-relay', (data) => {
          this._handleUseRelay(data);
        });
        
        this.socket.on('relay-ready', (data) => {
          this._handleRelayReady(data);
        });
  ...
  async _handleOffer(data) {
    ...
    try {
      ...
      this.socket.emit('answer', {
        ...
        useRelay: this.relayMode.get(data.userId) || false
      });
    } catch (error) {
  ...
  _createPeerConnection(userId) {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        this.socket.emit('ice-candidate', {
          ...
          useRelay: this.relayMode.get(userId) || false
        });
      }
    };
    ...
    pc.onconnectionstatechange = () => {
      ...
      // If connection fails, try using relay
      if (pc.connectionState === 'failed' && !this.relayMode.get(userId)) {
        console.log(`Direct connection to ${userId} failed, switching to relay mode`);
        this._requestRelay(userId);
      }
      
      // Clear connection timer if connected
      if (pc.connectionState === 'connected' && this.connectionTimers.has(userId)) {
    ...
    pc.oniceconnectionstatechange = () => {
      ...
      // If ICE connection fails, try using relay
      if (pc.iceConnectionState === 'failed' && !this.relayMode.get(userId)) {
        console.log(`ICE connection to ${userId} failed, switching to relay mode`);
        this._requestRelay(userId);
      }
    };
    ...
    // Set a timer to check if connection is established
    this.connectionTimers.set(userId, setTimeout(() => {
      if (pc.connectionState !== 'connected' && !this.relayMode.get(userId)) {
        console.log(`Connection to ${userId} timed out, switching to relay mode`);
        this._requestRelay(userId);
      }
  ...
  /**
   * Handle use relay event
   * @param {Object} data - Relay data
   * @private
   */
  _handleUseRelay(data) {
    console.log(`Received request to use relay for connection with ${data.userId}`);
    
    // Set relay mode for this user
    this.relayMode.set(data.userId, true);
    
    // Trigger relay mode event
    this._triggerEvent(EVENT.RELAY_MODE, { userId: data.userId });
    ...
    // Create new peer connection in relay mode
    const pc = this._createPeerConnection(data.userId);
    
    // If we are anchor, create and send offer
    if (this.isAnchor && this.localStream) {
      this._createAndSendOffer(data.userId);
    }
  }
  ...
  /**
   * Handle relay ready event
   * @param {Object} data - Relay data
   * @private
   */
  _handleRelayReady(data) {
    console.log(`Relay ready for connection with ${data.targetId}`);
    
    // Set relay mode for this user
    this.relayMode.set(data.targetId, true);
    
    // Trigger relay mode event
    this._triggerEvent(EVENT.RELAY_MODE, { userId: data.targetId });
    
    // Close existing peer connection if any
    if (this.peerConnections.has(data.targetId)) {
      const pc = this.peerConnections.get(data.targetId);
      pc.close();
      this.peerConnections.delete(data.targetId);
    }
    
    // Create new peer connection in relay mode
    const pc = this._createPeerConnection(data.targetId);
    
    // If we are anchor, create and send offer
    if (this.isAnchor && this.localStream) {
      this._createAndSendOffer(data.targetId);
    }
  }

  /**
   * Request relay for connection with user
   * @param {string} userId - User ID
   * @private
   */
  _requestRelay(userId) {
    if (!this.roomId || !this.userId) return;
    
    console.log(`Requesting relay for connection with ${userId}`);
    
    // Trigger relay mode event locally before server confirms
    this._triggerEvent(EVENT.RELAY_MODE, { userId: userId });
    
    this.socket.emit('request-relay', {
      roomId: this.roomId,
      userId: this.userId,
      targetId: userId
    });
